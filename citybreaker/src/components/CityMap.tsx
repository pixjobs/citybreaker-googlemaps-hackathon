...
function useAnimatedPanel(panelRef: React.RefObject<HTMLDivElement | null>, isOpen: boolean) {
  ...
  useEffect(() => {
    ...
    gsap.to(panelRef.current, {
      ...
      autoAlpha: isOpen ? 1 : 0, duration: 0.5, ease: "power3.inOut",
      onStart: () => { panelRef.current!.style.pointerEvents = isOpen ? "auto" : "none"; },
    });
  }, [isOpen, panelRef, isMobile]);
}

export default function CityMap({
  center, tripLength = 3, onPlacesLoaded, isItineraryOpen, onCloseItinerary,
  isSatelliteView, showLandmarks, showRestaurants, highlightedLocation,
  onMapLoad, onMapIdle,
}: CityMapProps) {
  ...
  const fetchAndShowPlaceDetails = useCallback(async (placeId: string) => {
    ...
    setIsDetailsLoading(true); setIsVideosLoading(true); setIsPopupOpen(true);
    setSelectedPlaceBasic(undefined); setSelectedPlaceDetails(undefined);
    setYoutubeVideos([]); setActiveTab("info");
    const service = new window.google.maps.places.PlacesService(mapRef.current);
    service.getDetails({ placeId, fields: ["name", "geometry", "photos", "formatted_address", "website", "rating", "reviews", "editorial_summary"] }, async (place, status) => {
      if (status !== window.google.maps.places.PlacesServiceStatus.OK || !place) {
        setIsPopupOpen(false); setIsDetailsLoading(false); setIsVideosLoading(false); return;
      }
      ...
      setSelectedPlaceDetails({ website: place.website, rating: place.rating, reviews: validReviews, editorial_summary: (place as PlaceResultWithSummary).editorial_summary });
      try {
        const res = await fetch("/api/youtube-search", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ query: place.name }) });
        const data = await res.json();
        setYoutubeVideos(data.videos || []);
      } catch { console.error("YouTube fetch failed"); }
      finally { setIsVideosLoading(false); setIsDetailsLoading(false); }
    });
  }, [isLoaded]);
  ...
  const searchAndDisplayPlaces = useCallback((placeType: string, markersRef: React.MutableRefObject<google.maps.Marker[]>, iconUrl: string) => {
    ...
    const service = new window.google.maps.places.PlacesService(mapRef.current);
    service.nearbySearch({ location: mapRef.current.getCenter()!, radius: 15000, type: placeType }, (results, status) => {
      if (status === window.google.maps.places.PlacesServiceStatus.OK && results) {
        results.forEach((p) => {
          if (!p.geometry?.location || !p.name || !p.place_id) return;
          const marker = new window.google.maps.Marker({ position: p.geometry.location, map: mapRef.current!, title: p.name, icon: { url: iconUrl, scaledSize: new google.maps.Size(35, 35) } });
          markersRef.current.push(marker);
          marker.addListener("click", () => fetchAndShowPlaceDetails(p.place_id!));
        });
      }
    });
  }, [isLoaded, clearMarkers, fetchAndShowPlaceDetails]);

  // Main Effect for Map Initialization and Core Logic
  useEffect(() => {
    if (!isLoaded) return;
    setPlacePhotos([]); clearMarkers(attractionMarkersRef); clearMarkers(landmarkMarkersRef); clearMarkers(restaurantMarkersRef);
    if (!mapRef.current) {
      const map = new window.google.maps.Map(document.getElementById("map") as HTMLElement, { center: { lat: center.lat, lng: center.lng }, zoom: center.zoom, disableDefaultUI: true, styles: retroDarkStyle });
      mapRef.current = map;
      onMapLoad?.(map);
      map.addListener("idle", () => { onMapIdle?.(); });
      map.addListener("click", (e: google.maps.IconMouseEvent) => {
        if (e.placeId) { e.stop(); fetchAndShowPlaceDetails(e.placeId); }
      });
    } else {
    ...
    service.nearbySearch({ location: new window.google.maps.LatLng(center.lat, center.lng), radius: 20000, type: "tourist_attraction" }, (results, status) => {
      if (status === window.google.maps.places.PlacesServiceStatus.OK && results) {
        ...
        results.forEach((p) => {
          if (!p.geometry?.location || !p.name || !p.place_id) return;
          const marker = new window.google.maps.Marker({ position: p.geometry.location, map: mapRef.current!, title: p.name, icon: { url: "/marker.png", scaledSize: new google.maps.Size(40, 40) } });
          attractionMarkersRef.current.push(marker);
          marker.addListener("click", () => fetchAndShowPlaceDetails(p.place_id!));
        });
      }
    });
    // ✅ THE FINAL FIX: The dependency array for this effect MUST include
    // onMapLoad and onMapIdle. Without them, React uses stale callback
    // functions from the first render, and the state in the parent never updates.
  }, [isLoaded, center, onPlacesLoaded, clearMarkers, fetchAndShowPlaceDetails, onMapLoad, onMapIdle]);

  useEffect(() => {
    if (!mapRef.current) return; const isSat = mapRef.current.getMapTypeId() === "satellite";
    if (isSatelliteView && !isSat) mapRef.current.setMapTypeId("satellite");
    else if (!isSatelliteView && isSat) { mapRef.current.setMapTypeId("roadmap"); mapRef.current.setOptions({ styles: retroDarkStyle }); }
  }, [isSatelliteView]);

  useEffect(() => {
    if (!mapRef.current || !isLoaded) return; if (highlightMarkerRef.current) { highlightMarkerRef.current.setMap(null); highlightMarkerRef.current = null; }
    if (highlightedLocation) {
      highlightMarkerRef.current = new window.google.maps.Marker({ position: highlightedLocation, map: mapRef.current, icon: { url: "/highlight-marker.png", scaledSize: new google.maps.Size(60, 60), anchor: new google.maps.Point(30, 60) }, animation: window.google.maps.Animation.DROP, zIndex: 999 });
    }
  }, [highlightedLocation, isLoaded]);

  useEffect(() => {
    if (showLandmarks) searchAndDisplayPlaces("landmark", landmarkMarkersRef, "/landmark-marker.png");
    else clearMarkers(landmarkMarkersRef);
  }, [showLandmarks, searchAndDisplayPlaces, clearMarkers]);

  useEffect(() => {
    if (showRestaurants) searchAndDisplayPlaces("restaurant", restaurantMarkersRef, "/restaurant-marker.png");
    else clearMarkers(restaurantMarkersRef);
  ...
  return (
    <>
      ...
      <div ref={panelRef} className="fixed inset-0 z-20 opacity-0 pointer-events-none" style={{ transform: "translateY(100%)" }}>
        {shouldOpenItinerary && (<ItineraryPanel cityName={center.name} places={placePhotos} onClose={onCloseItinerary} tripLength={tripLength} />)}
      </div>
    </>
  );
}