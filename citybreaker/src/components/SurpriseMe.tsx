...
export default function SurpriseMe({ isOpen, onClose, city, onZoomToLocation }: SurpriseMeProps) {
  ...
  const fetchSuggestions = useCallback(async (prompt: 'hungry' | 'entertain' | 'surprise') => {
    ...
    try {
      const res = await fetch('/api/surprise-me', {
        ...
        body: JSON.stringify({ prompt, city: { name: city.name, lat: city.lat, lng: city.lng } }),
      });
      if (!res.ok) {
  ...
  return (
    <div ref={panelRef} className="fixed inset-0 z-50 flex items-center justify-center bg-black/80 backdrop-blur-sm p-4 invisible">
      <div className="relative w-full max-w-md bg-[#0f0f0f] text-yellow-200 rounded-xl shadow-2xl border border-yellow-500/30 min-h-[32rem] flex flex-col">
        <button onClick={onClose} className="absolute top-3 right-3 text-gray-400 hover:text-yellow-200 transition-colors z-20"><FaTimes size={20} /></button>
        {viewState === 'loading' && ( <div className="flex-grow flex flex-col items-center justify-center text-center p-6"> <FaSpinner size={40} className="animate-spin text-yellow-400" /> <p className="mt-4 text-sm tracking-widest animate-pulse">Consulting the Oracle...</p> </div> )}
        <div style={{ display: viewState === 'prompts' ? 'flex' : 'none' }} className="flex-grow"> <div ref={promptContainerRef} className="flex flex-col items-center justify-center text-center p-6 space-y-4 w-full"> <h2 className="text-2xl font-bold tracking-wider text-yellow-300 mb-4">What’s Your Mission?</h2> {error && <p className="text-red-400 text-sm mb-4">{error}</p>} <button onClick={() => handlePromptClick('hungry')} className="prompt-button"><FaUtensils /><span>I’m Hungry</span></button> <button onClick={() => handlePromptClick('entertain')} className="prompt-button"><FaTheaterMasks /><span>Entertain Me</span></button> <button onClick={() => handlePromptClick('surprise')} className="prompt-button"><FaDice /><span>Surprise Me</span></button> </div> </div>
        
        {viewState === 'result' && suggestions && (
          <div ref={resultContainerRef} className="invisible flex-grow flex flex-col">
            <div className="relative border-b border-yellow-500/20 px-2 sm:px-4">
              <div className="flex justify-around">
                {suggestions.map((_, index) => ( <button key={index} ref={el => tabRefs.current[index] = el} onClick={() => handleTabClick(index)} className={`flex-1 py-3 text-sm font-semibold transition-colors duration-300 ${activeTabIndex === index ? 'text-yellow-300' : 'text-gray-500 hover:text-yellow-400'}`}> Option {index + 1} </button> ))}
              </div>
              <div ref={tabIndicatorRef} className="absolute bottom-0 h-0.5 bg-yellow-300 rounded-full" />
            </div>

            <div className="flex-grow overflow-y-auto">
              {currentSuggestion && (
                <div ref={contentRef}>
                  <div className="relative w-full h-48 sm:h-56"> <Image key={currentSuggestion.name} src={currentSuggestion.photoUrl} alt={currentSuggestion.name} fill className="object-cover" sizes="(max-width: 768px) 100vw, 500px" priority={activeTabIndex === 0}/> <div className="absolute inset-0 bg-gradient-to-t from-black/80 to-transparent" /> </div>
                  <div className="p-4 sm:p-6 space-y-4">
                    <h3 className="text-xl sm:text-2xl font-bold text-yellow-300">{currentSuggestion.name}</h3>
                    <p className="text-sm text-gray-300 leading-relaxed">{currentSuggestion.description}</p>
                    <div className="suggestion-section"> <h4 className="suggestion-section-header"><FaStar /> Why It’s Worth It</h4> <p>{currentSuggestion.whyWorthIt}</p> </div>
                    <div className="suggestion-section"> <h4 className="suggestion-section-header"><FaMapSigns /> Getting There</h4> <p>{currentSuggestion.transportInfo}</p> </div>
                  </div>
                </div>
              )}
            </div>
            ...
            <div className="p-4 border-t border-yellow-500/20">
              ...
              <div className="flex items-center gap-2 sm:gap-4 mb-4">
                  ...
                  <a href={currentSuggestion.tripAdvisorUrl} target="_blank" rel="noopener noreferrer" className="link-button"><FaBookOpen/><span>TripAdvisor</span></a>
              </div>
              <div className="flex justify-end"> <button onClick={handleTryAgain} className="flex items-center gap-2 text-sm text-gray-400 hover:text-yellow-200 transition-colors"><FaRedo /> Try Again</button> </div>
            </div>
          </div>
        )}
      </div>
