...
const ItineraryPanel: React.FC<ItineraryPanelProps> = ({
	...
	const generateExtendedItineraryPDF = useCallback(async () => {
		...
		try {
			...
			if (!res.ok) {
				const errorResponse = await res.json().catch(() => ({}));
				throw new Error(typeof errorResponse?.error === "string" ? errorResponse.error : "PDF generation failed");
			}
			const blob = await res.blob();
			saveAs(blob, `${safeCityName.replace(/\s+/g, "_")}_${currentTripLength}d_Itinerary.pdf`);
		} catch (err: unknown) {
	...
	return (
		<div className="fixed inset-0 z-40 bg-black/80 backdrop-blur-sm" onClick={onClose}>
			<div ref={panelRef} className="no-scrollbar absolute bottom-0 left-0 right-0 top-[80px] flex flex-col border-t border-neutral-700/50 bg-neutral-900 font-sans text-neutral-200 shadow-2xl sm:left-1/2 sm:top-[88px] sm:w-full sm:max-w-4xl sm:-translate-x-1/2 sm:rounded-2xl sm:border" onClick={(e) => e.stopPropagation()}>
				<header className="sticky top-0 z-20 flex items-center justify-between border-b border-neutral-700/50 bg-neutral-900/80 p-3 backdrop-blur-md sm:p-4">
					<div className="header-element flex items-center gap-1 rounded-full border border-neutral-700 bg-neutral-800 p-1">
						{TRIP_LENGTH_OPTIONS.map((days) => (
							<button key={days} onClick={() => handleTripLengthChange(days)} disabled={panelLoading} className={`rounded-full px-3 py-1 text-sm font-semibold transition-colors duration-200 disabled:cursor-not-allowed ${currentTripLength === days ? "bg-amber-400 text-black" : "text-neutral-300 hover:bg-neutral-700/50"}`}>
								{days} Days
							</button>
						))}
					</div>
					...
					<div className="flex items-center gap-2">
						{/* ✅ DEFINITIVE FIX: The text-based pill button is restored. */}
						...
						<button onClick={onClose} className="header-element rounded-full p-2 text-neutral-300 transition-colors hover:bg-neutral-700 hover:text-white" aria-label="Close itinerary panel">
							<X size={24} />
						</button>
					</div>
				</header>
								...
								<div key={day.title} className="day-block space-y-4 sm:space-y-5">
									<div className="relative h-18 w-full overflow-hidden rounded-xl border-2 border-amber-300/20 sm:h-24 sm:rounded-2xl">
										...
										<div className="absolute bottom-0 left-0 p-3 sm:p-5">
											...
											<h3 className="mt-0.5 font-serif text-base leading-tight text-white sm:text-xl">{day.title}</h3>
										</div>
									</div>
									<div className="space-y-4 sm:space-y-5">
										{day.activities.map((activity) => (
											<ActivityCard
												key={activity.title}
												activity={activity}
												place={enrichedPlaces.find((p) => p.name === activity.placeName)}
